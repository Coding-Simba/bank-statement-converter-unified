...
if trans2:
    print('\nAll transactions from enhanced parser:')
    ...
    # Summary
    deposits = sum(t.get('amount', 0) for t in trans2 if t.get('amount', 0) > 0)
    withdrawals = sum(t.get('amount', 0) for t in trans2 if t.get('amount', 0) < 0)
    total = sum(t.get('amount', 0) for t in trans2)
    print('-' * 70)
    print(f"{'Total Deposits':<56} ${deposits:>11.2f}")
    print(f"{'Total Withdrawals':<56} ${withdrawals:>11.2f}")
    print(f"{'Net Total':<56} ${total:>11.2f}")