#!/usr/bin/env python3
"""
Update blog pages with consistent navigation header
"""
...
def update_blog_page(filepath):
    """Update a single blog page with new navigation"""
    with open(filepath, 'r', encoding='utf-8') as f:
        content = f.read()
    
    # Check if navigation CSS already exists
    if 'Header Navigation - matching index.html' not in content:
        # Add navigation CSS before closing </style> tag if there's an inline style section
        if '</style>' in content:
...
def main():
    ...
    for filename in os.listdir(blog_dir):
        if filename.endswith('.html'):
            ...
            update_blog_page(filepath)
    
    print("Blog navigation update complete!")
